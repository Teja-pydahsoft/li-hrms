...
export default function LeaveReportsTab() {
    ...
    return (
        <div className="space-y-6 w-full pb-10">
            {/* Header Description */}
            <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-5 dark:bg-indigo-950/20 dark:border-indigo-900/30">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 relative z-0">
                <div className="bg-white rounded-2xl border border-slate-200 p-5 dark:bg-slate-900 dark:border-slate-800 shadow-sm">
                    <div className="flex items-center justify-between mb-4 pb-2 border-b border-slate-50 dark:border-slate-800">
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Total Leaves</span>
                        <div className="p-2 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl">
                            <FileText className="h-4 w-4 text-indigo-600" />
                        </div>
                    </div>
                    <p className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter">{reportStats.totalLeaves}</p>
                    <p className="mt-1 text-[10px] font-bold text-slate-400 uppercase tracking-tight">Total applications in period</p>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5 dark:bg-slate-900 dark:border-slate-800 shadow-sm">
                    ...
                    <p className="text-3xl font-black text-emerald-600 tracking-tighter">{reportStats.totalApprovedLeaves}</p>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5 dark:bg-slate-900 dark:border-slate-800 shadow-sm">
                    ...
                    <p className="text-3xl font-black text-amber-600 tracking-tighter">{reportStats.totalPendingLeaves}</p>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5 dark:bg-slate-900 dark:border-slate-800 shadow-sm">
                    ...
                    <p className="text-3xl font-black text-rose-600 tracking-tighter">
                        {reportStats.totalLeaves - (reportStats.totalApprovedLeaves + reportStats.totalPendingLeaves)}
                    </p>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/50 dark:bg-slate-800/30">
                                ...
                                <th className="px-5 py-3 text-[10px] font-black text-slate-500 uppercase tracking-widest text-right">Days</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {reportData.length > 0 ? reportData.map((leave) => (
                                <tr key={leave._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                                    ...
                                    <td className="px-5 py-3">
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-black text-slate-700 dark:text-slate-300">
                                                {dayjs(leave.fromDate).format('DD MMM')} - {dayjs(leave.toDate).format('DD MMM')}
                                            </span>
                                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter mt-0.5">
                                                {dayjs(leave.fromDate).format('YYYY')}
                                            </span>
                                        </div>
                                    </td>
                                    <td className="px-5 py-3">
                                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest ${
                                            leave.status === 'approved' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/30' :
                                            leave.status === 'rejected' ? 'bg-rose-50 text-rose-600 dark:bg-rose-950/30' :
                                    ...
                                    <td className="px-5 py-3 text-right">
                                        <span className="text-xs font-black text-slate-900 dark:text-white">{leave.numberOfDays}</span>
                                    </td>
                                </tr>
                            )) : (
                                <tr>
